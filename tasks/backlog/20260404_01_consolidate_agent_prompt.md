...
`agents/stc.md` contains static copies of `rules/first-principles.md` and `rules/task-management.md`. The `load-rules` skill loads them dynamically via `!cat`. Two sources of truth that can drift. Additionally, the agent prompt carries only rules — it does not carry decision heuristics, context management policy, or error recovery guidance that should persist across all skills and survive compaction.
...
- Update `load-rules` skill to remain consistent with the chosen single-source approach
...
- AC6: `/stc:load-rules` loads the same content as the agent mechanism — no behavioral difference between the two loading paths
- AC7: Agent prompt stays under 3000 words to leave context room for skills and user work

## Quality Gates

- Run `/stc:load-rules` and compare loaded context against agent system prompt — must be equivalent
